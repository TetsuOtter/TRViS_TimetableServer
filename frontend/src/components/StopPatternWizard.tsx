...
const StepIndicator = ({ step, labels }: StepIndicatorProps) => {
	return (
...
const Step2 = ({ data, setData, stations, stationsOnLine, t }: Step2Props) => {
				...
				<table
					...
					<tbody>
						{rangeStations.map((st, i) => {
							...
							const dwellDis = row.isPass || isFirst || isLast;
							...
							return (
								<tr
									...
									<td
										...
										<input
											type="checkbox"
											checked={!!row.isOperationOnlyStop}
											onChange={(e) => {
												updateRow(
													st.id,
													"isOperationOnlyStop",
...
const Step3 = ({ data, stations, lines, t }: Step3Props) => {
...
export const StopPatternWizard = ({
	...
	const handleFinish = () => {
		const id = isEdit && editPattern ? editPattern.id : "sp" + Date.now();
