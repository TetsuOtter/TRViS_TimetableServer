...
final class PagingQueryValidator
{
	...
	public static function withRequest(
		...
		if ($hasP) {
			// M4: strict integer-only parse. is_numeric()+intval() is lenient
			// (intval('1e9')===1000000000 on PHP 7.1+), so reject anything that
			// is not a pure-digit string, and enforce a MAX so a single request
			// cannot drive an unbounded (p-1)*perPage SQL OFFSET.
			...
			$p = intval($p);
			if ($p < Constants::PAGE_MIN_VALUE || Constants::PAGE_MAX_VALUE < $p) {
