...
use dev_t0r\trvis_backend\model\ApiErrorData;
use OpenApi\Attributes\Schema;
use PHPUnit\Framework\TestCase;
use ReflectionClass;

/**
 * Drift guard for ApiErrorData (per-entity, cloned from ProjectDriftTest —
 * the Phase-3 fan-out template). Asserts the #[OA\Schema] attribute stays in
 * lock-step with OAS_PROPERTIES / OAS_REQUIRED.
 */
class ApiErrorDataDriftTest extends TestCase
{
	...
	{
		$rc = new ReflectionClass(ApiErrorData::class);
		$schemaAttrs = $rc->getAttributes(Schema::class);
		$this->assertCount(1, $schemaAttrs, 'ApiErrorData must carry exactly one #[OA\\Schema]');
		...
		$this->assertSame(
			self::constArray(ApiErrorData::class, 'OAS_PROPERTIES'),
			$attrNames,
			'#[OA\\Schema(properties:)] set/order must equal ApiErrorData::OAS_PROPERTIES',
		);
	}
	...
	public function testSchemaRequiredMatchesOasRequiredConst(): void
	{
		...
		$this->assertSame(
			self::constArray(ApiErrorData::class, 'OAS_REQUIRED'),
			$required,
			'#[OA\\Schema(required:)] must equal ApiErrorData::OAS_REQUIRED',
		);
	}
}
