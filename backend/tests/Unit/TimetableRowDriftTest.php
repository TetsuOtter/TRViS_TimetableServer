...
/**
 * Drift guard for TimetableRow (per-entity, cloned from TrainDriftTest — the
 ...
 * (never enum values).
 */
class TimetableRowDriftTest extends TestCase
{
	...
	{
		$rc = new ReflectionClass(TimetableRow::class);
		$schemaAttrs = $rc->getAttributes(Schema::class);
		$this->assertCount(1, $schemaAttrs, 'TimetableRow must carry exactly one #[OA\\Schema]');
		...
		$this->assertSame(
			self::constArray(TimetableRow::class, 'OAS_PROPERTIES'),
			$attrNames,
			'#[OA\\Schema(properties:)] set/order must equal TimetableRow::OAS_PROPERTIES',
		);
	}
	...
	public function testSchemaRequiredMatchesOasRequiredConst(): void
	{
		...
		$this->assertSame(
			self::constArray(TimetableRow::class, 'OAS_REQUIRED'),
			$required,
			'#[OA\\Schema(required:)] must equal TimetableRow::OAS_REQUIRED',
		);
	}
}
