...
/**
 * Drift guard for Train (per-entity, cloned from WorkDriftTest — the
 ...
 * (never enum values).
 */
class TrainDriftTest extends TestCase
{
	...
	{
		$rc = new ReflectionClass(Train::class);
		$schemaAttrs = $rc->getAttributes(Schema::class);
		$this->assertCount(1, $schemaAttrs, 'Train must carry exactly one #[OA\\Schema]');
		...
		$this->assertSame(
			self::constArray(Train::class, 'OAS_PROPERTIES'),
			$attrNames,
			'#[OA\\Schema(properties:)] set/order must equal Train::OAS_PROPERTIES',
		);
	}
	...
	public function testSchemaRequiredMatchesOasRequiredConst(): void
	{
		...
		$this->assertSame(
			self::constArray(Train::class, 'OAS_REQUIRED'),
			$required,
			'#[OA\\Schema(required:)] must equal Train::OAS_REQUIRED',
		);
	}
}
