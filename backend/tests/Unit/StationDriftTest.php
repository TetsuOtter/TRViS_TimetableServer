...
/**
 * Drift guard for Station (per-entity, cloned from ProjectDriftTest — P3
 ...
 * or example values).
 */
class StationDriftTest extends TestCase
{
	...
	{
		$rc = new ReflectionClass(Station::class);
		$schemaAttrs = $rc->getAttributes(Schema::class);
		$this->assertCount(1, $schemaAttrs, 'Station must carry exactly one #[OA\\Schema]');
		...
		$this->assertSame(
			self::constArray(Station::class, 'OAS_PROPERTIES'),
			$attrNames,
			'#[OA\\Schema(properties:)] set/order must equal Station::OAS_PROPERTIES',
		);
	}
	...
	public function testSchemaRequiredMatchesOasRequiredConst(): void
	{
		...
		$this->assertSame(
			self::constArray(Station::class, 'OAS_REQUIRED'),
			$required,
			'#[OA\\Schema(required:)] must equal Station::OAS_REQUIRED',
		);
	}
}
