...
/**
 * Drift guard for TRViSJsonWork (per-entity, cloned from WorkDriftTest — the
 ...
 */
class TRViSJsonWorkDriftTest extends TestCase
{
	...
	{
		$rc = new ReflectionClass(TRViSJsonWork::class);
		$schemaAttrs = $rc->getAttributes(Schema::class);
		$this->assertCount(1, $schemaAttrs, 'TRViSJsonWork must carry exactly one #[OA\\Schema]');
		...
		$this->assertSame(
			self::constArray(TRViSJsonWork::class, 'OAS_PROPERTIES'),
			$attrNames,
			'#[OA\\Schema(properties:)] set/order must equal TRViSJsonWork::OAS_PROPERTIES',
		);
	}
	...
	public function testSchemaRequiredMatchesOasRequiredConst(): void
	{
		...
		$this->assertSame(
			self::constArray(TRViSJsonWork::class, 'OAS_REQUIRED'),
			$required,
			'#[OA\\Schema(required:)] must equal TRViSJsonWork::OAS_REQUIRED',
		);
	}
}
