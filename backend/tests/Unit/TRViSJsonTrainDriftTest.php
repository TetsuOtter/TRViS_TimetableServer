...
/**
 * Drift guard for TRViSJsonTrain (per-entity, cloned from WorkDriftTest — the
 ...
 * (never enum/example values).
 */
class TRViSJsonTrainDriftTest extends TestCase
{
	...
	{
		$rc = new ReflectionClass(TRViSJsonTrain::class);
		$schemaAttrs = $rc->getAttributes(Schema::class);
		$this->assertCount(1, $schemaAttrs, 'TRViSJsonTrain must carry exactly one #[OA\\Schema]');
		...
		$this->assertSame(
			self::constArray(TRViSJsonTrain::class, 'OAS_PROPERTIES'),
			$attrNames,
			'#[OA\\Schema(properties:)] set/order must equal TRViSJsonTrain::OAS_PROPERTIES',
		);
	}
	...
	public function testSchemaRequiredMatchesOasRequiredConst(): void
	{
		...
		$this->assertSame(
			self::constArray(TRViSJsonTrain::class, 'OAS_REQUIRED'),
			$required,
			'#[OA\\Schema(required:)] must equal TRViSJsonTrain::OAS_REQUIRED',
		);
	}
}
