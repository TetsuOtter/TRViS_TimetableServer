...
/**
 * Drift guard for StopPatternRow (per-entity, cloned from ProjectDriftTest).
 * Asserts the #[OA\Schema] attribute that drives swagger-php stays in
 * lock-step with the OAS_PROPERTIES / OAS_REQUIRED consts that drive the
 * runtime model. Only property NAMES + required are checked (never enum
 * values). No DB, no network — runs in the Unit suite.
 */
class StopPatternRowDriftTest extends TestCase
{
	...
	{
		$rc = new ReflectionClass(StopPatternRow::class);
		$schemaAttrs = $rc->getAttributes(Schema::class);
		$this->assertCount(1, $schemaAttrs, 'StopPatternRow must carry exactly one #[OA\\Schema]');
		...
		$this->assertSame(
			self::constArray(StopPatternRow::class, 'OAS_PROPERTIES'),
			$attrNames,
			'#[OA\\Schema(properties:)] set/order must equal StopPatternRow::OAS_PROPERTIES',
		);
	}
	...
	public function testSchemaRequiredMatchesOasRequiredConst(): void
	{
		...
		$this->assertSame(
			self::constArray(StopPatternRow::class, 'OAS_REQUIRED'),
			$required,
			'#[OA\\Schema(required:)] must equal StopPatternRow::OAS_REQUIRED',
		);
	}
}
